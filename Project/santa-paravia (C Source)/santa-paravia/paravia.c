...
struct Player
{
	int Cathedral, Clergy, CustomsTax, CustomsTaxRevenue, DeadSerfs;
...
{
	Me->Treasury += (Me->JusticeRevenue + Me->CustomsTaxRevenue);
...
void GenerateIncome(player *Me)
{
	...
	y = 150.0 - (float)Me->SalesTax - (float)Me->CustomsTax -
	(float)Me->IncomeTax;
	if(y < 1.0)
		y = 1.0;
	y /= 100.0;
	Me->CustomsTaxRevenue = Me->Nobles * 180 + Me->Clergy * 75 +
	Me->Merchants * 20 * y;
	Me->CustomsTaxRevenue += (int)(Me->PublicWorks * 100.0);
	Me->CustomsTaxRevenue = (int)((float)Me->CustomsTax / 100.0 *
		(float)Me->CustomsTaxRevenue);
	...
	revenues = Me->CustomsTaxRevenue + Me->SalesTaxRevenue +
...
/* Whoa. Slow down there son. */
	...
	if(z > 0.25)
		z = z / 10.0 + 0.25;
	zp = 50.0 - (double)Me->CustomsTax - (double)Me->SalesTax -
	(double)Me->IncomeTax;
	if(zp < 0.0)
	...
	else
	{
		...
		if((Me->CustomsTax + Me->SalesTax) < 35)
